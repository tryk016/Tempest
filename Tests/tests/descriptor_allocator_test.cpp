...
TEST(main, ShaderLayoutDescriptorCounts) {
  ShaderReflection::LayoutDesc layout;
  EXPECT_EQ(layout.size(),0u);
  EXPECT_EQ(layout.numResources(),0u);
  EXPECT_EQ(layout.numSamplers(),0u);
  layout.active=(1u<<31)|8u|1u;
  layout.resources=layout.active;
  layout.samplers=(1u<<31)|1u;
  layout.array=1u;
  EXPECT_EQ(layout.size(),3u);
  EXPECT_EQ(layout.numResources(),2u);
  EXPECT_EQ(layout.numSamplers(),1u);
  }
